...
always @(posedge dot_clk)
	if (!odd)	upper_word	<= #2 dat_i [31:16];

always @(posedge dot_clk)
	if (hblank || vblank)
		{r_o, g_o, b_o}	<= #2 0;
	else begin
		b_o	<= #2 {odd ? upper_word [15:11] : dat_i [15:11], 3'b000};
		g_o	<= #2 {odd ? upper_word [10:5] : dat_i [10:5], 2'b00};
		r_o	<= #2 {odd ? upper_word [4:0] : dat_i [4:0], 3'b000};
	end

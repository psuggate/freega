...
RESULTS_FILE	= 'results_pcmfsr.csv'

COUNTER_BITS	= range(5, 33)
COUNTER_PREFIX	= 'pcmfsr'
#COUNTER_BITS	= [5]
...
def runTest(bits):
	...
	makeMakefile(bits, name)
	makeUCF(bits, name)
	makeTopfile(bits, name)
	makePCtest(bits, name)
...
def makeMakefile(bits, name):
	...
	skel	= map(lambda x: x.replace('TEST_MODULE_XX', 'mfsrs/mfsr'+str(bits)+'.v pctest'), skel)
...
def makePCtest(bits, name):
	fi	= open('pctest.v', 'r')
	skel	= fi.readlines()
	fi.close()
	
	skel	= map(lambda x: x.replace('MFSR_XX', 'mfsr'+str(bits)), skel)
	
	fo	= open(name+'/pctest.v', 'w')
	fo.writelines(skel)
	fo.close()


CLOCK_PERIOD_BASE	= 3.2
CARRYMUX_DELAY		= 0.0
ucf	= '''

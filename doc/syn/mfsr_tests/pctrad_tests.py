...
RESULTS_FILE	= 'results_pctrad.csv'

COUNTER_BITS	= range(5, 33)
COUNTER_PREFIX	= 'pctrad'
#COUNTER_BITS	= [5]
...
def makeMakefile(name):
	...
	skel	= map(lambda x: x.replace('TEST_MODULE_XX', 'nullmfsr.v pctest'), skel)
...
def makeTopfile(bits, name):
	...
	skel	= map(lambda x: x.replace('USEMFSR_XX', '0'), skel)
...
def makePCtest(name):
	...
	skel	= map(lambda x: x.replace('MFSR_XX', 'nullmfsr'), skel)

...
RESULTS_FILE	= 'results_hybrid.csv'

COUNTER_BITS	= range(8, 33)
TRAD_BITS	= 3
COUNTER_PREFIX	= 'hybrid'
#COUNTER_BITS	= [5]
...
def runTest(bits):
	...
	makeMakefile(bits-TRAD_BITS, name)
	makeUCF(bits, name)
	makeTopfile(bits, name)
	makePChybrid(bits, name)
...
def makeMakefile(bits, name):
	fi	= open('Makefile.skel', 'r')
	skel	= fi.readlines()
	fi.close()
	
	skel	= map(lambda x: x.replace('TEST_MODULE_XX', 'mfsrs/mfsr'+str(bits)+'.v pchybrid'), skel)
...
def makeTopfile(bits, name):
	fi	= open('pctop_skel.v', 'r')
	skel	= fi.readlines()
	fi.close()
	
	bits	= str(bits)
	skel	= map(lambda x: x.replace('WIDTH_XX', bits), skel)
	skel	= map(lambda x: x.replace('USEMFSR_XX', '1'), skel)
	skel	= map(lambda x: x.replace('pctest', 'pchybrid'), skel)
	
	fo	= open(name+'/top.v', 'w')
	fo.writelines(skel)
	fo.close()

def makePChybrid(bits, name):
	fi	= open('pchybrid.v', 'r')
	skel	= fi.readlines()
	fi.close()
	
	skel	= map(lambda x: x.replace('TRADBITS_XX', str(TRAD_BITS)), skel)
	skel	= map(lambda x: x.replace('MFSR_XX', 'mfsr'+str(bits-TRAD_BITS)), skel)
	
	fo	= open(name+'/pchybrid.v', 'w')
	fo.writelines(skel)
	fo.close()


CLOCK_PERIOD_BASE	= 2.8
CARRYMUX_DELAY		= 0.3
ucf	= '''
...
def makeUCF(bits, name):
	...
	period	= CLOCK_PERIOD_BASE + CARRYMUX_DELAY*float(TRAD_BITS)
	fo	= open(ucfname, 'w')
